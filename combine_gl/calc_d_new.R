...
calc_d <- function(study, data, num_sdx_r2d = 2) {
  ...
  for (i in 1:length(data)) {
    ...
    # check if effect map is a t2 value by checking if study$orig_stat_type is equal to "t2"
      ...
      # convert to d
      this_t2 <- study[[i]]$orig_stat
      this_n1 <- study[[i]]$n1[1]
      this_n2 <- study[[i]]$n2[1]
